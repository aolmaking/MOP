...
async function openHistory(fixture: ComponentFixture<TechWorkCard>): Promise<HTMLElement> {
  ...
  // Both toggles, and only when they are actually shut. Clicking blindly
  // closed an already-expanded header on the second call, which is how the
  // car-switch test lost the panel halfway through.
  if (!element.querySelector('.live-details-expanded')) {
  ...
  if (!element.querySelector('.history-content')) {
