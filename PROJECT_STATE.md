...
3. **Close remaining partial page gaps only where their blockers are backed**: Access Denied and Password Reset are now built; Data Analyst Saved Views persistence is built; `Plan.allowedExports`/`analytics.export` now gates export permission correctly, while actual export file generation remains deferred.
...
- **Remaining page work is partial, not zero-implementation**: Data Analyst Export now has the `Plan.allowedExports` entitlement and `analytics.export` permission gate; it still needs the actual file-generation endpoint before the export half is complete. The other remaining page gaps are the partial rows already named in `docs/PAGE_INVENTORY.md`.
