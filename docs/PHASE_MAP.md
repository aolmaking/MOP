...
**Edge cases owed:** E20 (no documented database-failover recovery procedure — a config decision and a rehearsed runbook, not a feature). E12 fixed (session expiry now computed by Postgres, never an application replica's clock) — see `EDGE_CASE_REGISTER.md`.
